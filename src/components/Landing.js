...
const Landing = () => {
  ...
  const [language, setLanguage] = useState(languageOptions[languageOptions.length - 9]);
    ...
    axios
      ...
      .catch((err) => {
        ...
        if (status === 429) {
          ...
          showErrorToast(
            `Quota of 100 requests exceeded for the Day! Please subscribe for RAPID API Judge0!`,
            10000
          );
        }
        setProcessing(false);
        ...
        {/* <div className="px-5 py-1">
          <button
            ...
            className={classnames(
              "mt-4 border-2 border-black z-10 rounded-md shadow-[5px_5px_0px_0px_rgba(0,0,0)] px-8 py-2 hover:shadow transition duration-200 bg-white flex-shrink-0",
              !code ? "opacity-50" : ""
            )}
          >
            Dashboard
